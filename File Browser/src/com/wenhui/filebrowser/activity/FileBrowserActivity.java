...
public class FileBrowserActivity extends RootActivity {
	...
	protected void initView() {
		...
		if ( DeviceInfo.hasHoneycomb() ) {
			...
			mButtonBack.setOnClickListener( new OnBackButtonPress() );
			mButtonHome.setOnClickListener( new OnHomeButtonClick() );
			searchButton.setOnClickListener( new OnSearchButtonClick() );
		}
	}
	...
	private void openFile( File file ) {
		...
		if ( file.isDirectory() ) {
			if ( file.canRead() ) {
				...
				Toast.makeText( FileBrowserActivity.this, R.string.permission_denial, Toast.LENGTH_LONG ).show();
			}
	...
	protected Dialog onCreateDialog( int id ) {
		...
		case DIALOG_SORT_BY:
			...
			return builder.setTitle( R.string.list_by ).setSingleChoiceItems( R.array.list_by, App.instance().getCurrentListBy(), new OnClickListener() {
		...
		case DIALOG_OPEN_FAIL:
			...
			return builder4.setTitle( R.string.open_file ).setMessage( R.string.open_file_msg ).setPositiveButton( R.string.ok, new OnClickListener() {
		...
		case DIALOG_VIEW_BY:
			...
			String[] viewOptions = new String[] { getString( R.string.list ), getString( R.string.thumbnail ), getString( R.string.list_with_detail ) };
		...
		case DIALOG_SEND_OPTION:
			...
			return builder9.setTitle( getString(R.string.option) ).setSingleChoiceItems( adapter01, -1, new OnClickListener() {
				...
				public void onClick( DialogInterface dialog, int which ) {
					...
					Intent intentSend = new Intent( FileBrowserActivity.this, ShareMultipleFilesActivity.class );
					intentSend.putExtra( ShareMultipleFilesActivity.APP_TO_SEND_PACKAGENAME, packagename );
	...
	private void rename( final File originalFile ) {
		...
		// if it is directory, highlight the whole file name, otherwise hight light only the file name,
		// not the extension
	...
	private void displaySearchResults( final List< File > files, String query ) {
		query = " \"" + query + "\"";
		if ( files.isEmpty() ) {
			Toast.makeText( FileBrowserActivity.this, getString( R.string.no_file_found ) + query, Toast.LENGTH_LONG ).show();
			return;
		}
	...
	private void showBeforeZipDialog( final File file ) {
		...
		builder.setTitle( R.string.zip_file_exist_warning ).setIcon( R.drawable.warning ).setMessage( msg ).setPositiveButton( R.string.overwrite, new OnClickListener() {
		...
		public void onClick( View v ) {
			...
			case FileModel.TO_COPY:
				...
				if ( FileUtils.checkIfParentFile( mCurrentDir, file ) ) {
					Toast.makeText( FileBrowserActivity.this, R.string.cannot_cpy_to_child, Toast.LENGTH_LONG ).show();
					return;
				}
